...
+ 默认使用cgroupfs配置，如果需要采用官方建议的systemd，请参考：[02-附.配置systemd.md]([https://github.com/wanilyer/kubeadm/blob/master/02-%E9%99%84.%E9%85%8D%E7%BD%AEsystemd.md](https://github.com/wanilyer/kubeadm/blob/master/02-附.配置systemd.md))
...
4. 集群初始化失败
   ...
   1. kubeadm 错误日志如下（使用--v=5 or higher 可看到更详细的日志）：
    ```
       Error writing Crisocket information for the control-plane node: timed out waiting for the condition
    ```

   2. `docker logs ${apiserver}` (apiserver容器ID)
	```
      grpc: addrConn.createTransport failed to connect to {https://10.142.97.136:2379 0  <nil>}. Err :connection error: desc = "transport: authentication handshake failed: x509: certificate is valid for 10.152.99.209, 127.0.0.1, ::1, 10.142.97.136, not 10.142.99.209". Reconnecting
   ```
